...
from mlisne import run_onnx_session, standardize, cumMean
...
def estimate_qps_onnx(onnx: str, X_c = None, X_d = None, data = None, C: Sequence = None, D: Sequence = None, L: Dict[int, Set] = None,
    ...
    delta: float/list, default: 0.8
        Radius of sampling ball. If list, then QPS is recomputed for each delta in list.
...
def _computeQPSNumba(onnx, X_c: np.ndarray, X_d: np.ndarray, L_inds: Tuple[np.ndarray, np.ndarray], L_vals: np.ndarray,
                types: Tuple[np.dtype, np.dtype], S: int, delta: float, mu: np.ndarray, sigma: np.ndarray,
                input_type: int, input_names: Tuple[str, str], fcn, cpu: bool, parallel: bool, **kwargs):
    """Compute QPS for a single row of data
    ...
    **kwargs: keyword arguments to pass into decision function
    ...
    if isinstance(delta, Sequence):
        multi_delta = True
        inference_draws_list = _drawQPS(X_c, standard_draws, u_draws, L_inds, L_vals, S, delta, mu, sigma, multi_delta)
    else:
        multi_delta = False
        inference_draws = _drawQPS(X_c, standard_draws, u_draws, L_inds, L_vals, S, delta, mu, sigma, multi_delta)
    ...
    # Return means of every S rows
    qps = cumMean(ml_out, S, multi_delta)
    return qps

@jit(parallel = True)
def _drawQPS(X_c: np.ndarray, standard_draws: np.ndarray, u_draws: np.ndarray, L_inds: Tuple[np.ndarray, np.ndarray],
            L_vals: np.ndarray, S: int, delta: float, mu: np.ndarray, sigma: np.ndarray, multi_delta: bool):
    nrows = X_c.shape[0]
    p_c = X_c.shape[1]
    na_inds = np.where(np.isnan(X_c))
    ...
    for i in range(standard_draws.shape[0]):
        for s in range(standard_draws.shape[1]):
            ...
            scaled = row/np.sqrt(np.sum(row[~np.isnan(row)]))
    ...
    if multi_delta == True:
        ...
        uniform_draws = scaled_draws * np.expand_dims(u, 2) * delta + np.expand_dims(X_c, 1) # Scale by sampled u and ball mean/radius to get the final uniform draws (nrow x S x p_c)
    ...
    # Collapse to 2D for inference
    if multi_delta == True:
        inference_draws = np.stack([d.reshape((nrows*S, p_c)) for d in destandard_draws])
    else:
        inference_draws = destandard_draws.reshape((nrows*S, p_c))
...
def estimate_qps_onnx_numba(onnx: str, X_c = None, X_d = None, data = None, C: Sequence = None, D: Sequence = None, L: Dict[int, Set] = None,
                      S: int = 100, delta: float = 0.8, seed: int = None, types: Tuple[np.dtype, np.dtype] = (None, None), input_type: int = 1,
                      input_names: Tuple[str, str]=("c_inputs", "d_inputs"), fcn = None, vectorized: bool = False, cpu: bool = False, iobound: bool = False,
                      parallel: bool = False, nprocesses: int = None, ntasks: int = 1, **kwargs):
    """Estimate QPS for given dataset and ONNX model
    ...
    if L is not None:
        ...
        mixed_og_vals, mixed_og_inds = None
    ...
    if parallel == True:
        ...
        computeQPS_frozen = partial(_computeQPSNumba, types = types, S = S, delta = delta, mu = mu, sigma = sigma, input_type = input_type,
                                    input_names = input_names, fcn = fcn, cpu = cpu, parallel = parallel, **kwargs)
        mp = pathos.helpers.mp
        p = mp.Pool(nprocesses)
        #p = pathos.pools._ProcessPool(nprocesses)
        ...
        qps_vec = _computeQPSNumba(onnx, X_c, X_d, mixed_og_inds, mixed_og_vals, types, S, delta, mu, sigma, input_type, input_names, fcn, cpu, parallel, **kwargs) # Compute QPS for each individual i
        qps_vec = np.array(qps_vec)
    gc.collect()
    return qps_vec
...
def _computeUserQPS(X_c: np.ndarray, X_d: np.ndarray, L_inds: np.ndarray, L_vals: np.ndarray, ml, S: int, delta: float, mu: np.ndarray, sigma: np.ndarray,
    ...
    """Compute QPS using a user-defined input function.
