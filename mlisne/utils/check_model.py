...
def check_model(original_model_path, onnx_model_path, inputs_path, model_type, tf_input_names, tf_output_names):
    import os
    ...
    if len(expected_outputs) > 0:
        ...
        try:
            if model_type == "tensorflow":
                expected_outputs = tfRunner(original_model_path, inputs_path, tf_input_names, tf_output_names)
            else:
    ...
    if type(expected_outputs) is list:
        ...
        for output_name in expected_outputs.keys():
            try:
                tmp_onnx_out = onnx_outputs[output_name]
            except Exception as e:
                print(e)
                print(
                ...
                return "UNSUPPORTED"
            # Need to check that ONNX output is flattened from dict
            from typing import Dict
            if isinstance(tmp_onnx_out[0], Dict):
                tmp_onnx_out = np.array([[d[key] for key in d.keys()] for d in tmp_onnx_out])
